...
<div class="max-w-[720px] mx-auto">
    ...
    <h2 class="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface mb-8">New Note</h2>
    ...
    <form method="POST" action="{{ route('notes.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="content" rows="15"
                class="w-full bg-surface-container-low border border-outline-variant rounded-lg p-5 font-body-md text-on-surface placeholder:text-outline-variant focus:outline-none focus:border-primary transition-colors"
                placeholder="Write your knowledge here...">{{ old('content') }}</textarea>
        </div>
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                ...
                <div class="flex gap-4" id="color-picker">
                    ...
                    <input type="hidden" name="color" id="color-input" value="{{ old('color') }}">
                    <label class="flex flex-col items-center gap-1 cursor-pointer group" onclick="document.getElementById('color-input').value='';document.querySelectorAll('#color-picker label .check').forEach(e=>e.style.display='none');document.querySelectorAll('#color-picker label .swatch').forEach(e=>e.style.outline='');this.querySelector('.check').style.display=''">
                        <div class="w-6 h-6 rounded-full border-2 border-dashed border-outline-variant flex items-center justify-center group-hover:scale-110 transition-transform swatch">
                            <span class="material-symbols-outlined text-[14px] text-primary check" style="display:{{ old('color') ? 'none' : '' }}">check</span>
                        </div>
                        <span class="text-[11px] font-medium text-secondary">None</span>
                    </label>
                    @foreach($colors as $name=>$hex)
                    @php $sel = old('color') === $name; @endphp
        ...
        <div class="flex gap-4 pt-4">
            <button type="submit" class="bg-primary text-on-primary px-8 py-3 rounded-lg font-label-sm text-label-sm hover:brightness-110 active:scale-95 transition-all">Save Note</button>
            <a href="{{ route('notes.index') }}" class="px-8 py-3 rounded-lg border border-outline-variant font-label-sm text-label-sm text-on-surface hover:bg-surface-container-low transition-all">Cancel</a>
        </div>
    </form>
</div>
@endsection
